...
function readSavFile(game, file) {
  ...
  for (const [area, secrets] of Object.entries(foundSecrets)) {
    console.log(
      `${camel2ui(area)}: ${secrets.total - secrets.found} missed of ${secrets.total} (${((secrets.found * 100) / secrets.total).toFixed()}% found)`
    );
    for (const secret of secrets.missed) console.log(`  ${secret}`);
  }
