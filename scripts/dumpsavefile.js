...
function readSavFile(game, file) {
  ...
  const pipecaps = {
    ...
    A5FastTravelPipe_ExitCombatFinale: [
      'DLC2_Complete:PipeToAreaBoss2',
      'DLC2_Complete:PipesystemNewDLC_CombatExitPipe',
    ],
...
*/
  ...
  for (let o of loadedSave.Properties) {
    ...
    for (let x of o.value.value) {
      if (x == 'None') continue;
      if (!markers[o.name]) markers[o.name] = [];
